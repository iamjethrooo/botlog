...
let CatCommand = class CatCommand extends framework_1.Command {
    chatInputRun(interaction) {
        axios_1.default
            .get(`https://api.tenor.com/v1/random?key=${process.env.TENOR_API}&q=cat&limit=1`)
            .then(async (response) => {
            ...
            .catch(async (error) => {
            console.error(error);
            return await interaction.reply('Something went wrong when trying to fetch a cute kitty gif :(');
        });
    }
    async messageRun(message) {
        axios_1.default
            ...
            .catch(async (error) => {
            console.error(error);
            return await message.reply('Something went wrong when trying to fetch a cute kitty gif :(');
        });
    }
